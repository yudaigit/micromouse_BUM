...
void fastRun1(short gx, short gy)
{
  ...
  while ((g_map_control.getMyPosX() != gx) || (g_map_control.getMyPosY() != gy)) {
    switch (g_map_control.getNextDir2(gx, gy, &glob_nextdir)) {
      ...
      case right:
        ...
        straight_count = 0;
        frontWallhosei(9,SEARCH_SPEED);
        slalom(right);
        oneStep(15, SEARCH_SPEED);
        break;
      case left:
        straight(straight_count * SECTION, SEARCH_SPEED, MAX_SPEED, SEARCH_SPEED);
        straight_count = 0;
...
void fastRun2(short gx, short gy)
{
  t_direction_glob glob_nextdir;
  int straight_count = 0;
  ...
  while ((g_map_control.getMyPosX() != gx) || (g_map_control.getMyPosY() != gy)) {
    switch (g_map_control.getNextDir2(gx, gy, &glob_nextdir)) {
      case front:
        ...
        break;
      case right:
        straight_faster(straight_count * SECTION, SEARCH_SPEED, 800, SEARCH_SPEED);
        straight_count = 0;
        frontWallhosei(9,SEARCH_SPEED);
        slalom(right);
        oneStep(15, SEARCH_SPEED);
